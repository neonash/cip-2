...
def amazon_all_info(kw_str1):
    global prod_title, rev_url, price, prod_img_src, prod_descr, prod_rating, browser, prod_URL, prods_info, kw_str, chrome_path, all_reviews, all_kw_all_rev, status_code
    kw_str = kw_str1
    ...
    try:
        ...
        try:
            ...
            count_of_result_pgs = 0  # to keep track of count of result pages scraped (only few result pages. not all)
            while has_next_page:
                count_of_result_pgs += 1
                try:
                    result_url = browser.current_url
                    result_urls_list.append(result_url)
                ...
                # Loop through all product URLs on current Result page
                #for u1 in range(len(prod_urls)):
                for u1 in range(0, 5):  # for limited products, second parameter value is excluded while looping
                    ...
                    try:
                        prod_rating = browser.find_element_by_id("acrPopover").get_attribute("title")
                        prod_rating = re.search(r'[0-9](.[0-9])?', prod_rating).group(0)
                    except:
                        prod_rating = "0.0"
                    ...
                    # WORKING CODE. TEMPORARILY UNCOMMENTED TO INCREASE LOOPS. DO NOT REMOVE.------------------------

                    if last_page_number >= 2:
                        # Loop from page 2 till last page, as page 1 is already added to reviews_urls_list, and then add it to list
                        # for i1 in range(2, last_page_number + 1):  # +1 because 2nd parameter is last index (excluded)
                        for i1 in range(2, 3):  # will run from 2 to 15
                ...
                # uncomment for only 1 review per product. Just for debugging.
                # has_next_page = False

                # if has_next_page:  # use when looping thru all result pages
                if has_next_page and count_of_result_pgs <= 3:  # can change value of count_of_result_pgs as per need.
                    print "Going to next Results page..."
                    # logging.info("Going to next Results page...")
